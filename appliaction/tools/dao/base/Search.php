...
class Search
{
    ...
    public function check($type = 0, $value = '')
    {
        ...
        $is = table('SearchDisable')->where($map)->field('id')->value();
        if (!$is) {
    ...
    public function addLog($uid = 0, $type = 0, $value = '')
    {
        ...
        $id = table('SearchLog')->where($map)->field('id')->value();
        if (!$id) {
    ...
    public function addRecommend($type = 0, $value = '')
    {
        ...
        //查询是否禁用
        $isDisable = table('SearchDisable')->where(array('type' => $type, 'value' => $value))->field('id')->value();
        if ($isDisable) {
            return array('status' => false, 'msg' => '关键词已被禁用不可推荐,请先删除禁用关键字');
        }

        $id = table('SearchRemmond')->where(array('type' => $type, 'value' => $value))->field('id')->value();
        if ($id) {
    ...
    public function addDisable($type = 0, $value = '')
    {
        ...
        //查询是否推荐
        $isDisable = table('SearchRemmond')->where(array('type' => $type, 'value' => $value))->field('id')->value();
        if ($isDisable) {
            return array('status' => false, 'msg' => '关键词已被推荐不可禁用,请先删除推荐关键字');
        }

        $id = table('SearchDisable')->where(array('type' => $type, 'value' => $value))->field('id')->value();
    ...
    public function del($table, $type = 0, $value = '')
    {
        ...
        $id = table($table)->where($map)->field('id')->value();
