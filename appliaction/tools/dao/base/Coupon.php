...
class Coupon
{
    ...
    public function lists($map, $lg = 'zh', $offer = 0, $pageSize = 1000)
    {
        ...
        $list = table('CouponLog')->join($coupon, "$coupon.id = $couponLog.coupon_id")->where($map)->limit($offer, $pageSize)->field($field)->order("$couponLog.id desc,$couponLog.use_time asc,$coupon.end_time desc")->select();
    ...
    public function canUseCouponList($uid, $price = 0)
    {
        ...
        $list  = table('CouponLog')->join($coupon, "$coupon.id = $couponLog.coupon_id", 'left')->where($map)->field($field)->order("$couponLog.use_time asc,$coupon.end_time desc")->select();
