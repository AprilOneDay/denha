...
class Sms
{
    ...
    public function send($mobile = '', $flag = '', $param = '', $method = 'get', $country = '')
    {
        ...
        //保存短信发送记录
        $this->savelog($mobile, $flag, $content);
    ...
    /**
     * 每日最大发送次数
     * @date   2018-04-03T15:54:57+0800
     * @author ChenMingjiang
     * @param  [type]                   $mobile [手机号]
     * @param  [type]                   $num    [发送次数]
     * @return [type]                           [description]
     */
    public function maxSendNum($mobile, $num = 10)
    {
        $todaystart = strtotime(date('Y-m-d' . '00:00:00', TIME)); //获取今天00:00
        $todayend   = strtotime(date('Y-m-d' . '00:00:00', TIME + 3600 * 24)); //获取今天24:00

        $map['mobile']  = $mobile;
        $map['created'] = ['between', [$todaystart, $todayend]];

        $count = table('SmsLog')->where($map)->count();
        if ($count >= $num) {
            return ['status' => false, 'msg' => '每日最多可接受' . $num . '条短信'];
        }
     ...
     * @param  [type]                   $content [description]
     */
    public function savelog($mobile, $flag, $content)
    {
