...
class Message
{
     ...
     * @param  [type]                   $options    [附加参数]
     * @return [type]                               [description]
     */
    public function send($toUid = 0, $flag = '', $param = [], $options = [])
    {
        if (!$toUid) {
            return false;
        }

        $uid    = isset($options['uid']) ? $options['uid'] : 0; // 发送信息uid
        $type   = isset($options['type']) ? $options['type'] : 1; // 站内信类型
        $lg     = isset($options['lg']) ? $options['lg'] : 'zh'; // 推送语言类型
        $isPush = isset($options['isPush']) ? $options['isPush'] : false; //是否开启推送
        ...
        $map['uid']    = $uid;
        $map['to_uid'] = $toUid;
        $map['param']  = $param ? json_encode($param, JSON_UNESCAPED_UNICODE) : '';

        $map['del_status'] = 0;

        $id = table('UserMessage')->where($map)->field('id')->value();
        if ($id) {
    ...
    public function getList($lg = 'zh', $mapValue = array())
    {
        ...
        $list = table('UserMessage')->where($map)->select();
