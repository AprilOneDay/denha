...
class Collection
{
    ...
    public function add($uid = 0, $type = 0, $value = '')
    {
        ...
        $is = table('Collection')->where(array('uid' => $uid, 'type' => $type, 'value' => $value, 'del_status' => 0))->field('id')->value();
        if ($is) {
    ...
    public function del($uid = 0, $type = 0, $id = 0)
    {
        ...
        $collectionId = table('Collection')->where(array('value' => $id, 'uid' => $uid, 'type' => $type, 'del_status' => 0))->field('id')->value();
        if (!$collectionId) {
