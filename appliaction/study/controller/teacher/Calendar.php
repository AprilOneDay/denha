...
class Calendar extends \app\study\controller\Init
{
    ...
    public function index()
    {
        ...
        $listTmp = table('UserCourseLog')->where($map)->group('start_time')->find('array');
        foreach ($listTmp as $key => $value) {

            $value['goods']      = table('OrdersCourse')->where('order_sn', $value['order_sn'])->field('title,btitle,class_time,class_time_en,sign,num')->find();
