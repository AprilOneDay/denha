    ...
    public function lists()
    {
        ...
        if ($param['field'] && $param['keyword']) {
            if ($param['field'] == 'content') {
                ...
                $uid = table('User')->where(array('nickname' => array('instr', $param['keyword'])))->field('id')->value();
                if ($uid) {
                    $map['uid'] = $uid;
                }
        ...
        if ($param['to_field'] && $param['to_keyword']) {
            $toUid = table('User')->where(array('nickname' => array('instr', $param['to_keyword'])))->field('id')->value();
            if ($toUid) {
                $map['to_uid'] = $toUid;
            }
        }

        $list  = table('ChatLog')->where($map)->limit($offer, $pageSize)->order('id desc')->select();
