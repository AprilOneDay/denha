...
class ExamLog extends Init
{
    public function lists()
    {
        ...
        $list  = table('UserExamLog')->where($map)->limit($offer, $pageSize)->order('id desc')->select();
        ...
        foreach ($list as $key => $value) {
            ...
            $list[$key]['total_score'] = (int) table('ExamData')->where($map)->field('SUM(score) AS score')->value();
