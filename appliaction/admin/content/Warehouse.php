...
class Warehouse extends Init
{
    public function lists()
    {
        ...
        $list  = table('WarehouseInfo')->where($map)->limit($offer, $pageSize)->order('id desc')->select();
        $total = table('WarehouseInfo')->where($map)->count();
        $page  = new Pages($total, $pageNo, $pageSize, url('', $param));

        $abc = table('Column')->limit($offer, $pageSize)->select();

        foreach ($list as $key => $value) {
            $map            = array();
            $map['bname_2'] = array('in', "$value[country_id],$value[category_id]");

            $house = table('Category')->where($map)->field('name')->column();
    ...
    public function getCategory()
    {
        ...
        $id   = table('Category')->where('bname_2', $id)->id->value();
